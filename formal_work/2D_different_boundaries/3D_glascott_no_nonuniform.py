...
np.savetxt(
            f"formal_work/data2D/k0/no_oxide/glascott/x2_nodes_uniform.dat",
            x2_nodes,
        )
...
while t < tmax:
                    ...
                    if (j == 0) or (j == n2 - 1):
                        if j == 0:
                            ...
                            if (m_i > 0) and (m_i <= radial_value):
                                ...
                                deriv_cg = (-3 * c2[0,m_i] + 4 * c2[1,m_i] - c2[2,m_i]) / (2 * h2) 
                        ...
                        if c2[j,m_i] > cs:
                            ...
                            val += [
                                ...
                                * (c2[j,m_i] - c2[j - 1,m_i])
                                / (h2s),  # D_{j-1,m_i}
                                -0.5
                                * ((D[j + 1,m_i] + D[j,m_i]) + (D[j,m_i] + D[j - 1,m_i]))
                                / (h2s)
                                -0.5
    ...
    if step % 20 == 0:
        ...
        np.savetxt(
            f"formal_work/data2D/k0/no_oxide/glascott/c2_201_uniform_thalf{t:.2f}.dat",
            c2,
        )
        np.savetxt(

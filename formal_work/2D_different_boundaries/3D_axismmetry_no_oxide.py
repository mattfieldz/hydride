...
np.savetxt(
            f"formal_work/data2D/k0/no_oxide/x2_nodes.dat",
            x2_nodes,
        )
...
while t < tmax:
                    ...
                    if (j == 0) or (j == n2 - 1):
                        if j == 0:
                            ...
                            if (m_i > 0) and (m_i < 6):
                                ...
                                val += [-3 * D[j,m_i] / (2 * h2 * x2d_nodes[0]) - D3/L3,   
                                        4 * D[j,m_i] / (2 * h2 * x2d_nodes[0]),
                                        -D[j,m_i] / (2 * h2 * x2d_nodes[0])]
                                b += [D3 * (c2[0,m_i] - 1) / L3 - D[j,m_i] * deriv_cg]
    ...
    if step % 10 == 0:
        ...
        np.savetxt(
            f"formal_work/data2D/k0/no_oxide/c2{t:.2f}.dat",
            c2,
        )
        np.savetxt(
            f"formal_work/data2D/k0/no_oxide/alpha{t:.2f}.dat",
            alpha,
        )
        # np.savetxt(

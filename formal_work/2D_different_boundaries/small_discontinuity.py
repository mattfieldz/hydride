...
np.savetxt(
            f"formal_work/data2D/k1e6/zero_flux/x2_nodes.dat",
            x2_nodes,
        )
np.savetxt(
            f"formal_work/data2D/k1e6/zero_flux/x3_nodes.dat",
            x3_nodes,
        )
...
while t < tmax:
    ...
    while residual > tol:
        ...
        for m_i in range(0,m):
            ...
            if (17 < m_i) and (m_i < 22):
                ...
                b += [(3 * c2[0,m_i] - 4 * c2[0 + 1,m_i] + c2[0 + 2,m_i])]
    ...
    if step % 100 == 0:
        ...
        np.savetxt(
            f"formal_work/data2D/k1e6/zero_flux/c3{t:.2f}.dat",
            c3,
        )
        np.savetxt(
            f"formal_work/data2D/k1e6/zero_flux/c2{t:.2f}.dat",
            c2,
        )
        np.savetxt(
            f"formal_work/data2D/k1e6/zero_flux/alpha{t:.2f}.dat",
            alpha,
        )
        # np.savetxt(

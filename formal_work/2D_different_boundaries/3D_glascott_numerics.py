...
L2star = 200 * 1e3 * 1.0e-7  # bulk domain lengthscale 1000um
L3star = 20 * 1.0e-7  # oxide domain lengthscale 20nm
Lmstar = 200 * 1e3 * 1.0e-7 # 500um
...
# Choose computation coord Mmax such that x_from_X(Xmax)=L2
...
# nodes in the oxide and bulk
M2_nodes = np.linspace(0, Mmax, m)  # bulk nodes in comp. coordinate
# physical node locations
...
np.savetxt(
            f"formal_work/data2D/k0/no_oxide/glascott/x2_nodes_non_uniform.dat",
            x2_nodes,
        )
...
while t < tmax:
            ...
            for j in range(0, n2):
                ...
                if m_i == 0:
                    ...
                    if (j == 0) or (j == n2 - 1):
                        ...
                        m2dph = 0.5 * (m2d_nodes[m_i + 1] + m2d_nodes[m_i])
                        m2dmh = 0.5 * (m2d_nodes[m_i] + m2d_nodes[m_i - 1])
                        ...
                        if c2[j,m_i] > cs:
                            ...
                            val += [
                                ...
                                * ((D[j + 1,m_i] + D[j,m_i]) / x2dph + (D[j,m_i] + D[j - 1,m_i]) / x2dmh)
                                / (h2s * x2d_nodes[j])
                                -0.5
                                * ((D[j,m_i+1] + D[j,m_i]) / m2dph + (D[j,m_i] + D[j,m_i-1]) / m2dmh)
                                / (hls * m2d_nodes[m_i]) # left ride contributions
                                - 2 / dt
                                ...
                                * (c2[j,m_i] - c2[j - 1,m_i])
                                ...
                                +0.5
                                * (c2[j,m_i+1] - c2[j,m_i]) 
                                / (hls * m2d_nodes[m_i] * m2dph) # left side contributions
                                
                                - 0.5
                                * (c2[j,m_i] - c2[j,m_i-1])
                                / (hls * m2d_nodes[m_i] * m2dmh)

                                + (c2[j,m_i+1] - c2[j,m_i-1])/(m_r * m2d_nodes[m_i])
                                ,# D_j
                                0.5
                                * (D[j + 1,m_i] + D[j,m_i])
                                ...
                                * (c2[j + 1,m_i] - c2[j,m_i])
                                ...
                                0.5
                                * (D[j,m_i+1] + D[j,m_i])
                                / (hls * m2d_nodes[m_i] * m2dph) 
                                +
                                (D[j,m_i])/(m_r * m2d_nodes[m_i]), # right node
                                0.5
                                * (D[j,m_i] + D[j,m_i-1])
                                / (hls * m2d_nodes[m_i] * m2dmh)
                                +
                                (D[j,m_i])/(m_r * m2d_nodes[m_i]), # left node
                                +0.5
                                * (c2[j, m_i+1] - c2[j,m_i])
                                / (hls * m2d_nodes[m_i] * m2dph),  # D_{j+1} right node
                                +0.5
                                * (c2[j,m_i] - c2[j,m_i-1])
                                / (hls * m2d_nodes[m_i] * m2dmh)  # D_{j+1} left node
                            ...
                            b += [
                                ...
                                * (c2[j,m_i] - c2[j,m_i-1])
                                ...
                                + D[j,m_i] * (c2[j,m_i + 1] - c2[j,m_i-1]) / (m_r * m2d_nodes[m_i])
                            ...
                            val += [
                                ...
                                * (D[j,m_i+1] + D[j,m_i]) # right node
                                / (hls * m2d_nodes[m_i] * m2dph)
                                +
                                (D[j,m_i])/(m_r * m2d_nodes[m_i]),  # c_{j+1}
                                ...
                                * (D[j,m_i] + D[j,m_i-1]) # left node
                                ...
                                +0.5
                                * (c2[j,m_i+1] - c2[j,m_i])
                                / (hls * m2d_nodes[m_i] * m2dph),  # D_{j+1}
                            ...
                            b += [
                                ...
                                * (c2_old[j,m_i] - c2_old[j,m_i-1])
                                / (hls * m2d_nodes[m_i] * m2dmh)
                                + D_old[j,m_i] * (c2_old[j,m_i + 1] - c2_old[j,m_i-1]) / (m_r * m2d_nodes[m_i])
                            ]
    ...
    if step % 10 == 0:
        ...
        np.savetxt(
            f"formal_work/data2D/k0/no_oxide/glascott/c2_non_uniform_201{t:.2f}.dat",
            c2,
        )
        np.savetxt(

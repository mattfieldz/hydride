...
def x_from_X(X):
    ...
    """
    BX = 0.1  # smaller BX leads to more NON-uniformity
...
L2star = 100 * 1e3 * 1.0e-7  # bulk domain lengthscale 100um
L3star = 20 * 1.0e-7  # oxide domain lengthscale 20nm
Lmstar = 75 * 1e3 * 1.0e-7 # 500um
...
np.savetxt(
            f"formal_work/data2D/k0/axis_deep/x2_nodes.dat",
            x2_nodes,
        )
np.savetxt(
            f"formal_work/data2D/k0/axis_deep/x3_nodes.dat",
            x3_nodes,
        )
...
while t < tmax:
    ...
    while residual > tol:
        ...
        for m_i in range(0,m):
            ...
            if (m_i > 0) and (m_i < 6):
                ...
                col += [0+m_s]
                val += [1]
                b += [1 - np.exp(-t) - c3[0,m_i]]  

            else:
                # at the last node we impose no H flux out of the domain
                            
                row += 3 * [0 + m_s]
                col += [0 + m_s + 2,0 + m_s + 1, 0 + m_s]
                val += [-1, 4, -3]
                # second order differencing
                b += [(3 * c3[0,m_i] - 4 * c3[0 + 1,m_i] + c3[0 + 2,m_i])]
            ...
            for j in range(1, n3 - 1):
                if m_i == m-1:
                    ...
                    val += [D3 / h3s, -2 * D3 / h3s - 2 * D3 / hls - 2 / dt, D3 / h3s, D3 / hls + D3/(m_r), D3/hls - D3/(m_r)]
                    ...
                    b += [
                        ...
                        - D3 * (c3[j,m_i + 1] - c3[j,m_i-1]) / m_r

                        - D3 * (c3_old[j,m_i-1] - 2 * c3_old[j,m_i] + c3_old[j,m_i+1]) / hls
                        - D3 * (c3_old[j,m_i+1] - c3_old[j,m_i-1])/m_r
    ...
    if step % 100 == 0:
        ...
        np.savetxt(
            f"formal_work/data2D/k0/axis_deep/c3{t:.2f}.dat",
            c3,
        )
        np.savetxt(
            f"formal_work/data2D/k0/axis_deep/c2{t:.2f}.dat",
            c2,
        )
        np.savetxt(
            f"formal_work/data2D/k0/axis_deep/alpha{t:.2f}.dat",
            alpha,
        )
        # np.savetxt(

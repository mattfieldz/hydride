...
while t < tmax:
    ...
    if step % 100 == 0:
        print(
            f"Plot at t={t} tstar={t*(Lrefstar**2)/(Drefstar*eps*reactK**0.25)} (sec) c_int={c2[0]} a_int={alpha[0]} c_end={c2[n2-1]} itn={iteration}"
        )
        
        np.savetxt(
            f"formal_work/data1D/asymptotic/c2_1001_{t:.5f}.dat",

...
while t < tmax:
        ...
        for j in range(0, n2):
            ...
            if (j == 0) or (j == n2 - 1):
                if j == 0:
                    ...
                    b += [
                        1-c2[j]  # deriv in terms of comp. coord.
                    ]
                if j == n2 - 1:
    ...
    if step % 50 == 0:
        V = Lrefstar*(hydride_interface[int(step/10)-1]-hydride_interface[int(step/10)-2])/((10*dt)*tscaling)
        V_list.append(V)
        t_list.append(t*tscaling)
        V_array = np.array(V_list)
        print(
            f"Plot at t={t} tstar={t*tscaling} (sec) c_int={c2[0],alpha[0]} int={Lrefstar*hydride_interface[int(step/10)-1]} V={V} c_end={c2[n2-1]} itn={iteration}"
        )
        # plt.plot(t_list,V_list)
        # plt.plot(t_list,np.sqrt(D2star*kstar*(Castar-Csstar)**2/(3*N2star))/(1-threshold_alpha)*np.ones(len(t_list)))
        
        V_asymp = np.sqrt(2*D2star*kstar*(Castar-Csstar)**(m+1)/(3*N2star*(m+1)))/(1-threshold_alpha)*np.ones(len(t_list))
        # plt.plot(t_list,np.abs(V_array-V_asymp)/(V_array))

        numerics_lenth_hydride.append(hydride_interface[int(step/10)-1])
        ...
        # np.savetxt(
        #     f"formal_work/data1D/c2_condon_098{t:.2f}.dat",

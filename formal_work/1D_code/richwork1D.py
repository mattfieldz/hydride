...
tmax = 3.0e7 * Drefstar / (Lrefstar * Lrefstar)
...
# Initial conditions
c3 = np.loadtxt(f'formal_work/data1D/k1e15/c31000.00.dat')[:,1]
c2 = np.loadtxt(f'formal_work/data1D/k1e15/c21000.00.dat')[:,1]
alpha = np.loadtxt(f'formal_work/data1D/k1e15/alpha1000.00.dat')[:,1]

D = alpha * D2 + (1-alpha) * D1
...
while t < tmax:
    ...
    if step % 2000 == 0:
        ...
        np.savetxt(
            f"formal_work/data1D/k1e15/c3{t:.2f}.dat",
            np.transpose(np.array([x3_nodes, c3])),
        )
        np.savetxt(
            f"formal_work/data1D/k1e15/c2{t:.2f}.dat",
            np.transpose(np.array([x2_nodes, c2])),
        )
        np.savetxt(
            f"formal_work/data1D/k1e15/alpha{t:.2f}.dat",

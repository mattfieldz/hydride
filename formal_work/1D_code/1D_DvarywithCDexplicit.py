...
while t < tmax:
        ...
        for j in range(0, n2):
            ...
            if (j == 0) or (j == n2 - 1):
                if j == 0:
                    ...
                    val += [
                        -3 * D_old[0] / (2 * h2 * x2d_nodes[0]),
                        4 * D_old[0] / (2 * h2 * x2d_nodes[0]),
                        -D_old[0] / (2 * h2 * x2d_nodes[0]),
                    ]
                ...
                if c2[j] > cs:
                    ...
                    val += [
                        ...
                        * (c2[j] - c2[j - 1])
                        ...
                        -0.5
                        * ((D_old[j + 1] + D_old[j]) / x2dph + (D_old[j] + D_old[j - 1]) / x2dmh)
                        / (h2s * x2d_nodes[j])
                        - 2 / dt * eps
            ...
            if alpha_old[j] < threshold_alpha:
                ...
                b += [-D_old[j] + np.max([D2, D_large * np.exp(1/(100*(alpha_old[j]-threshold_alpha)))])]

                # b += [-D[j] +np.max([D2 + D_large * np.exp(1/(100*(alpha_old[j]-threshold_alpha))),D2 * 10 * pressure])]
            else:
                row += [k+2]
                col += [k+2]
                val += [1]
    ...
    if step % 1000 == 0:
        ...
        # )
        print(
                    f"Plot at t={t:.7f} tstar={t*tscaling:.3f} (sec) c_int={cratio * c2[0]:.6f} a_int={alpha[0]:.6f} c_end={c2[n2-1]} D_int={D[0]:.4f} gamma={1e7 * Lrefstar * hydride_interface[-1]:.4f} itn={iteration} time={time.time()-computer_time_zero:.2f} vstar = {1e7 * Lrefstar * (hydride_interface[-1]-hydride_interface[-2]) / (100*dt*tscaling):.4f}"
                )
        t_list.append(t)
        t_array = np.array(t_list)
        np.savetxt(f"formal_work/data1D/lambdaP/tvalues_true_param.dat",
                t_array,
        )
        ...
        np.savetxt(
            f"formal_work/data1D/lambdaP/c2_condon_oxide_true_param{t:.7f}.dat",
            np.transpose(np.array([x2_nodes, c2])),
        )
        np.savetxt(
            f"formal_work/data1D/lambdaP/alpha_condon_oxide_true_param{t:.7f}.dat",

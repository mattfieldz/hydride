...
while t < tmax:
    ...
    while residual > tol:
        ...
        for j in range(0, n2):
            ...
            if (j == 0) or (j == n2 - 1):
                if j == 0:
                    ...
                    b += [
                        ...
                        - D[0]
                        * (-3 * c2[0] + 4 * c2[1] - c2[2])
                        / (2 * h2)  # deriv in terms of comp. coord.
                    ]
                if j == n2 - 1:
                ...
                if j == n2 -1:
                    k = j * nv
                    row += 4*[n2*nv]
                    col += [k,k-nv,k-2*nv,n2*nv]
                    val += [
                        3 * D[0] / (2 * h2),
                        -4 * D[0] / (2 * h2),
                        D[0] / (2 * h2 ),
                        cs * ((D2 * eps * reactK**0.5)/(np.pi*t))**0.5
                    ]
                    b += [-D[0] * (3 * c2[0] - 4 * c2[1] + c2[2]) / (2 * h2) 
                        -cs * Gamma * ((D2 * eps * reactK**0.5)/(np.pi*t))**0.5]
                ...
                val += [
                    ...
                    * (c2[j + 1] - c2[j])
                    / (h2s),  # D_{j+1}
                    -6 * Gamma * m * c2[j]**(m-1) * alpha[j] # Gamma
                ]
                ...
                b += [
                    ...
                    * (c2_old[j] - c2_old[j - 1])
                    / (h2s)
                    + 3 * Gamma_old**2 * ((c2_old[j]) ** m) * alpha_old[j]
                ]
            ...
            if (j==0) or (j==n2-1):
                if j==0:
                    ...
                    row += 5 * [k + 1]
                    col += [k, k + 1, k + 1 + nv , k + 1 + 2 * nv ,(n2)*nv]  # c_{j},alpha_j
                    val += [
                        Gamma * (m * (c2[j]) ** (m-1) * alpha[j]), # c_{j}
                        (c2[j]) ** m - 2 / dt - 2 * (Gamma-Gamma_old)/dt * (-3/(2*h2)) , # alpha_j
                        2 * (Gamma-Gamma_old)/dt * (4 / (2*h2)), # alpha_{j+1}
                        2 * (Gamma-Gamma_old)/dt * (-1 / (2*h2)), # alpha_{j+2}
                        2 * (alpha[j]-alpha_old[j])/dt
                        - 2/dt * (-3*alpha[j]+4*alpha[j+1]-alpha[j+2])/(2*h2)
                        + (c2[j]**m) * alpha[j] # Gamma
                    ]
                    # residuals for a Crank-Nicolson method
                    b += [
                        -2 * Gamma * (alpha[j] - alpha_old[j]) / dt
                        + 2*(Gamma-Gamma_old)/dt * (-3 * alpha[j] + 4 * alpha[j+1] - alpha[j+2])/(2*h2)
                        -Gamma * ((c2[j]) ** m) * alpha[j]
                        -Gamma_old * ((c2_old[j]) ** m) * alpha_old[j]
                    ]
                if j==n2-1:
                    row += 5 * [k + 1]
                    col += [k, k + 1, k + 1 - nv , k + 1 - 2 * nv ,(n2)*nv]  # c_{j},alpha_j
                ...
                row += 5 * [k + 1]
                col += [k, k + 1, k + 1 - nv , k + 1 + nv ,n2*nv]  # c_{j},alpha_j
                val += [
                    Gamma * (m * (c2[j]) ** (m-1) * alpha[j]), # c_j
                    Gamma * (c2[j]) ** m + 2 / dt, # alpha
                    -2 * (Gamma-Gamma_old)/dt * (-1 / (2*h2)), # alpha_{j-1}
                    -2 * (Gamma-Gamma_old)/dt * (1 / (2*h2)), # alpha_{j+1}
                    2 * (alpha[j]-alpha_old[j])/dt
                    - 2/dt *  Y_nodes[j] * (alpha[j+1]-alpha[j-1])/(2*h2)
                    + (c2[j]**m) * alpha[j] # Gamma
                ]
                # residuals for a Crank-Nicolson method
                b += [
                    -2 * Gamma * (alpha[j] - alpha_old[j]) / dt
                    + 2 * Y_nodes[j] *(Gamma-Gamma_old)/dt * (alpha[j+1]-alpha[j-1])/(2*h2)
                    -Gamma * ((c2[j]) ** m) * alpha[j]
                    -Gamma_old * ((c2_old[j]) ** m) * alpha_old[j]
                ]
            #
        ...
        a = sp.sparse.coo_matrix((val, (row, col)), shape=(n2 * nv+1,n2 * nv+1))

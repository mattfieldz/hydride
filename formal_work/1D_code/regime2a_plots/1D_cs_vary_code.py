...
D2star = 1.9e-2 * np.exp(-48387/(8.31*tempK))

D3star = 0.037 * np.exp(-60000/(8.31*tempK)) # cm^2/s, diffusion of H in UO2

N2star = 8.01e-2  # mol/cm^3, number density of U
Csstar = 0.25 * np.exp(-52000 / (8.31 * tempK)) # mol/cm^3, saturation [H] in U -- *ad-hoc* value
...
print('c+k Castar,',Castarck)
print('ddd,',Csstar)

kstar = 10.4/N2star * np.exp(1592/tempK) # reaction constant in cm^3/mol/s
...
print(Castar,'Castar')
print(Crefstar/Castar,'ratio of concentrations')
...
while t < tmax:
    ...
    while residual > tol:
        ...
        for j in range(0, n2):
            ...
            if alpha_old[j] < threshold_alpha:
                ...
                b += [-D[j] + D2 + D_large * np.exp(1/(mu*(alpha_old[j]-threshold_alpha)))]
    ...
    if step % 100 == 0:
        ...
        np.savetxt(f"formal_work/data1D/lambdaP/tvalues_vary_diff_cs0.dat",
                t_array,
        )

...
hydride_interface = []
hydride_interface_cs01 = []
hydride_interface_cs03 = []
hydride_interface_cs1 = []
...
for t in tvalues:
    ...
    alpha_tot_cs01 = np.loadtxt(
                f"formal_work/data1D/lambdaP/alpha_condon_oxide_vary_diff_cs01{t:.7f}.dat")
    alpha_cs01 = alpha_tot_cs01[:,1]
    interpolated_alpha_cs01 = sp.interpolate.CubicSpline(x2_nodes, alpha_cs01 - threshold_alpha)
    roots_cs01 = interpolated_alpha_cs01.roots(extrapolate=False)
    if len(roots_cs01) > 0:
        hydride_interface_cs01.append(roots_cs01[0])
    else:
        hydride_interface_cs01.append(0)

    alpha_tot_cs03 = np.loadtxt(
                f"formal_work/data1D/lambdaP/alpha_condon_oxide_vary_diff_cs03{t:.7f}.dat")
    alpha_cs03 = alpha_tot_cs03[:,1]
    interpolated_alpha_cs03 = sp.interpolate.CubicSpline(x2_nodes, alpha_cs03 - threshold_alpha)
    roots_cs03 = interpolated_alpha_cs03.roots(extrapolate=False)
    if len(roots_cs03) > 0:
        hydride_interface_cs03.append(roots_cs03[0])
    else:
        hydride_interface_cs03.append(0)

    alpha_tot_cs1 = np.loadtxt(
                f"formal_work/data1D/lambdaP/alpha_condon_oxide_vary_diff_cs0_Dinfty1e2{t:.7f}.dat")
    alpha_cs1 = alpha_tot_cs1[:,1]
    interpolated_alpha_cs1 = sp.interpolate.CubicSpline(x2_nodes, alpha_cs1 - threshold_alpha)
    roots_cs1 = interpolated_alpha_cs1.roots(extrapolate=False)
    if len(roots_cs1) > 0:
        hydride_interface_cs1.append(roots_cs1[0])
    else:
        hydride_interface_cs1.append(0)
...
gamma_cs01 = np.array(hydride_interface_cs01)
gamma_cs03 = np.array(hydride_interface_cs03)
gamma_D1e3 = np.array(hydride_interface_D1e3)
gamma_cs1 = np.array(hydride_interface_cs1)
...
# plt.plot(x2_nodes[0:n],alpha_pred[0:n],label=r'Predicted concentration',linestyle='solid',color='green')
# plt.plot(x2_nodes[0:n],alpha[0:n],label=r'Numerical concentration ($c_s=0$)',color='red',linestyle='dashed')
# plt.plot(x2_nodes[0:n],alpha[0:n],label=r'Numerical concentration ($c_s=0.1$)',color='red',linestyle='dashed')
# plt.plot(x2_nodes[0:n],alpha[0:n],label=r'Numerical concentration ($c_s=0.3$)',color='red',linestyle='dashed')
# plt.plot(x2_nodes[0:n],alpha[0:n],label=r'Numerical concentration ($c_s=1$)',color='red',linestyle='dashed')




# plt.plot(x2_nodes[0:n],alpha_pred[0:n],label=r'Predicted concentration',linestyle='solid',color='green')
# plt.plot(x2_nodes[0:n],alpha[0:n],label=r'Numerical concentration ($c_s=0$)',color='green',linestyle='dashed')



print('lol',t)
# plt.plot(gam_pred(t) * np.ones(100),np.linspace(0.88,1.02,100),color='blue',linestyle='solid',label=r'Predicted $\Gamma(\tau)$')
# plt.plot(gamma[-1] * np.ones(100),np.linspace(0.88,1.02,100),color='blue',linestyle='dashed',label=r'Predicted $\Gamma(\tau)$')
...
plt.plot(tvalues,gamma_pred,color='green',linestyle='solid')

plt.plot(tvalues,gamma,color='red',linestyle='dashed')
plt.plot(tvalues,gamma_cs01,color='red',linestyle='dashed')
plt.plot(tvalues,gamma_cs03,color='red',linestyle='dashed')
plt.plot(tvalues,gamma_cs1,color='red',linestyle='dashed')
...
# plt.plot(tvalues,gamma_pred,color='orange')
...
plt.savefig('formal_work/1D_code/regime2a_plots/figures_latex/cs-vary-gamma-plot.eps',format='eps',bbox_inches='tight')
# plt.savefig('formal_work/1D_code/regime2a_plots/pre-fracture.eps',format='eps',bbox_inches='tight')
# plt.savefig('formal_work/1D_code/regime2a_plots/D-vary-plot.eps',format='eps',bbox_inches='tight')
# plt.savefig('formal_work/1D_code/regime2a_plots/figures_latex/gamma-comp.eps',format='eps',bbox_inches='tight')

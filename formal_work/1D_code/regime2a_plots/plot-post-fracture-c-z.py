...
tvalues = np.loadtxt(f"formal_work/data1D/lambdaP/tvalues_vary_diff.dat")
# tvalues_cs = np.loadtxt(f"formal_work/data1D/lambdaP/tvalues_vary_diff_cs01.dat")
...
for t in tvalues:
    alpha_tot = np.loadtxt(
                f"formal_work/data1D/lambdaP/alpha_condon_oxide_vary_diff{t:.7f}.dat")
    c2_tot = np.loadtxt(
            f"formal_work/data1D/lambdaP/c2_condon_oxide_vary_diff{t:.7f}.dat")
...
def con_pred(t,x):
    if t < tau_c:
        return 1/np.sqrt(3) * np.exp(-np.sqrt(3)*x)
    if x < gam_pred(t):
...
for i in range(len(alpha_pred)):
    alpha_pred[i] = alph_pred(t,x2_nodes[i])
...
for i in range(len(conc_pred)):
    conc_pred[i] = con_pred(t,x2_nodes[i])


# plt.plot(x2_nodes[0:80],conc_pred[0:80],label=r'Predicted concentration',linestyle='dashed',color='blue')
# # plt.plot(x2_nodes[0:400],interpolated_c_cs(x2_nodes)[0:400],label=r'Numerical concentration ($c_s=0.1$)')
# plt.plot(x2_nodes[0:80],interpolated_c(x2_nodes)[0:80],label=r'Numerical concentration ($c_s=0$)',color='blue')

plt.plot(gam_pred(t) * np.ones(100),np.linspace(0.88,1.02,100),color='blue',linestyle='dashed',label=r'Predicted $\Gamma(\tau)$')


# plt.xlabel(r'$z$')
# plt.ylabel(r'$c-c_s$')
# conc_int_pred = np.copy(tvalues)
# for i in range(len(tvalues)):
#     conc_int_pred[i] = con_pred(tvalues[i],0)

plt.plot(x2_nodes[0:300],alpha[0:300],color='green',linestyle='dashed')
plt.plot(x2_nodes[0:300],alpha_pred[0:300],color='green',linestyle='solid')
# plt.plot(x2_nodes[0:300],alpha[0:300])
plt.savefig('formal_work/1D_code/regime2a_plots/figures_latex/post-fracture-a-tau1.eps',format='eps',bbox_inches='tight')

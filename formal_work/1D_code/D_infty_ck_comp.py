...
Castar = N2star * 4.13e-6 * np.exp(-894/tempK) * np.sqrt(1e5 * pressure)  # mol/cm^3, surface value for [H] -- *ad-hoc* value
...
lambdaP =  scale_up * 5e5 * pressure * np.exp(-5000/tempK)

print('lol',N2star,Castar,D2star,kstar)
...
while t < tmax:
        ...
        for j in range(0, n2):
            ...
            if (j == 0) or (j == n2 - 1):
                if j == 0:
                    ...
                    b += [
                        1-c2[0] # deriv in terms of comp. coord.
                    ]
                if j == n2 - 1:
            ...
            if alpha[j] < threshold_alpha:
                ...
                b += [-D[j] + D2 + D_large * np.exp(1/(100*(alpha[j]-threshold_alpha)))]
            else:
                row += [k+2]
                col += [k+2]
                val += [1]
    ...
    if step % 10 == 0:
        ...
        if len(roots) > 0:
            ...
            c2_val = interpolated_c2(roots[0])
    ...
    if step % 100 == 0:
        ...
        # )
        print(
                    f"Plot at t={t:.7f} tstar={t*tscaling:.3f} (sec) c_int={c2[0]:.6f} a_int={alpha[0]:.6f} c_end={c2[n2-1]:4f} D_int={D[0]:.4f} gamma={1e7 * Lrefstar * hydride_interface[-1]:.4f} itn={iteration} time={time.time()-computer_time_zero:.2f} vstar = {1e7 * Lrefstar * (hydride_interface[-1]-hydride_interface[-2]) / (10*dt*tscaling):.4f} c2 @ front = {c2_val:.4f}"
                )
        t_list.append(t)

...
D2star = 1.94e-2 * np.exp(-48387/(8.31*tempK))

D3star = 0.037 * np.exp(-60000/(8.31*tempK)) # cm^2/s, diffusion of H in UO2
...
Jstar = 1597 * np.sqrt(pressure) * np.exp(-88000/(8.31 * tempK))

Castar = Jstar * L3star / D3star

Crefstar = Jstar * Lrefstar / Drefstar


cratio = Crefstar/Castar


cs = Csstar / Crefstar  # saturation value \in [0,1)
eps = Crefstar / N2star  # relative forcing measure
...
h2 = L2 / (Xmax*(n2 - 1))
h2s = h2 * h2
dt_standard = 0.000025
dt = dt_standard
tol = 1.0e-8


# threshold_alpha = 0.99 - 0.1 * pressure
...
print(threshold_alpha,J,cs,eps,mu,Dlarge)
print('Lrefstar = ', Lrefstar)
print('trefstar = ', tscaling)
...
while t < tmax:
    ...
    while residual > tol:
        ...
        for j in range(0, n2):
            ...
            if c2[j] > cs:
                ...
                val += [
                    -reactK * (m * (c2[j] - cs) ** (m-1) * alpha[j]),
                    -reactK * (c2[j] - cs) ** m - 2 / dt,
                ]
                # residuals for a Crank-Nicolson method
                b += [
                    2 * (alpha[j] - alpha_old[j]) / dt
                    + reactK * ((c2[j] - cs) ** m) * alpha[j]
                    + reactK * ((c2_old[j] - cs) ** m) * alpha_old[j]
                ]
            else:
            ...
            ##############################
            #
            if alpha_old[j] < threshold_alpha:
                ...
                b += [-D[j] + D2 + Dlarge * np.exp(1/(mu*(alpha_old[j]-threshold_alpha)))]

                # b += [-D[j] + D2]


                # b += [-D[j] +np.max([D2 + D_large * np.exp(1/(100*(alpha_old[j]-threshold_alpha))),D2 * 10 * cratio * c2_old[j]])]
            else:
                row += [k+2]
                col += [k+2]
                val += [1]
                # b += [-D[j]+D2*(1/(1-c2[j]*5))]
                # b += [-D[j] + D2 * np.max([1,(1+c2_old[j]*cratio*100-3)])]
                

                b += [-D[j] + D2]
                # b += [-D[j] + D2 * (1 + Crefstar * 1e7 * c2_old[j])]
    ...
    if step % 200 == 0:
        ...
        # )
        # print(
        #             f"Plot at t={t:.7f} tstar={t*tscaling:.3f} (sec) c_int={c2[0]:.6f} a_int={alpha[0]:.6f} c_end={c2[n2-1]} D_int={D[0]:.4f} gamma={hydride_interface[-1]:.4f} itn={iteration} time={time.time()-computer_time_zero:.2f} vstar = {1e7 * Lrefstar * (hydride_interface[-1]-hydride_interface[-2]) / (100*dt*tscaling):.4f}"
        #         )
        print(
                    f"Plot at t={t:.7f} tstar={t*tscaling:.3f} (sec) c_int={c2[0]:.6f} a_int={alpha[0]:.6f} c_end={c2[n2-1]} D_int={D[0]:.4f} itn={iteration} time={time.time()-computer_time_zero:.2f} "
                )
        t_list.append(t)
        t_array = np.array(t_list)
        # np.savetxt(f"formal_work/data1D/lambdaP/tvalues_vary_diff_cs0_Dinfty1e1.dat",
        #         t_array,
        # )
        ...
        np.savetxt(
            f"formal_work/data1D/lambdaP/c2_condon_oxide_vary_diff_cs1{t:.7f}.dat",
            np.transpose(np.array([x2_nodes, c2])),
        )
        np.savetxt(
            f"formal_work/data1D/lambdaP/alpha_condon_oxide_vary_diff_cs1{t:.7f}.dat",

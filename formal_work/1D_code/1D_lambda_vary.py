...
while t < tmax:
    ...
    if step % 100 == 0:
        ...
        # )
        print(
                    f"Plot at t={t:.7f} tstar={t*tscaling:.3f} (sec) c_int={c2[0]:.6f} a_int={alpha[0]:.6f} c_end={c2[n2-1]} D_int={D[0]:.4f} gamma={1e7 * Lrefstar * hydride_interface[-1]:.4f} itn={iteration} time={time.time()-computer_time_zero:.2f} vstar = {1e7 * Lrefstar * (hydride_interface[-1]-hydride_interface[-2]) / (dt*tscaling):.4f}"
                )
        t_list.append(t)

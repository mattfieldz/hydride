...
# plt.plot(Trefstar * t_backstepping / 3600,Lrefstar * (x_backstepping) / 1e-7,label='Asymptotic solution')
# plt.plot(t_values * Trefstar / 3600 ,(np.sqrt(2/3 * eps * D1 * (1-cs) * t_values) - D1/D3 * L3) * Lrefstar / 1e-7,label='asy')
print(D1/D3*L3)
...
plt.xlabel('t^* / h')
plt.ylabel('Hydride thickness / nm')
plt.xlim(xmin=0)
plt.ylim(ymin=0)
plt.legend()
plt.show()
...
while t < tmax:
    ...
    # oxide region
        ...
        for j in range(1, n2 - 1):  # logical node in metal
            ...
            val += [D_neg_half / h2s, -(D_pos_half + D_neg_half) / h2s - 1 / dt - 9*k_2*c2_update_nodes[e1*j]**2*(1-c2_update_nodes[e1*j+1]),
                     D_pos_half / h2s]
            b += [-c2_nodes[e1 * j] / dt + 3*k_2*c2_update_nodes[e1*j]**3*(1-c2_update_nodes[e1*j+1])
            ...
            row += [e1 * k + 1]
            col += [e1 * k + 1]
            val += [1/dt + k_2 * c2_update_nodes[e1*j]**3]
            b += [c2_nodes[e1*j+1]/dt + k_2*c2_update_nodes[e1*j]**3]

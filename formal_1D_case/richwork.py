...
Lmstar = 1e5 * 1.0e-7 # 100um
D1star = 1.0e-12  # cm^2/s, diffusion of H in UH3  -- *ad hoc* value
...
reactK = (
    1.0e7  # ad-hoc value, SSI 2024 paper suggests 1.e^4-1.e^5 but based on 1nm scale!
)
...
while t < tmax:
    ...
    while residual > tol:
        ...
        for m_i in range(0,m):
            ...
            if (m_i == 0) or (m_i == m-1):
                if m_i == m-1:
                    ...
                    for i in range(0,n2-1):
                        ...
                        col += [(j-2*m_j) + m_s, (j-m_j) + m_s, (j)+m_s]
                        val += [-1, 4, -3]
                        b += [c2[i,m_i-2] + 3*c2[i,m_i] - 4*c2[i,m_i-1]]
            ...
            # surface has [H]=c3=1 for node 0
                ...
                for j in range(1, n3 - 1):
                    ...
                    val += [D3 / h3s, -4 * D3 / h3s - 2 / dt, D3 / h3s, D3 / hls, D3/hls]
                    # residuals for a Crank-Nicolson method
                    b += [
                        2 * (c3[j,m_i] - c3_old[j,m_i]) / dt
                        - D3 * (c3[j - 1,m_i] - 4 * c3[j,m_i] + c3[j + 1,m_i] + c3[j,m_i+1] + c3[j,m_i-1]) / h3s
                        - D3 * (c3_old[j - 1,m_i] - 4 * c3_old[j,m_i] + c3_old[j + 1,m_i] + c3_old[j,m_i+1] + c3_old[j,m_i-1]) / h3s
                    ]
                        ...
                        if c2[j,m_i] > cs:
                            ...
                            val += [
                                ...
                                * ((D[j,m_i+1] + D[j,m_i]) + (D[j,m_i] + D[j,m_i-1]))
                                ...
                                - 0.5
                                * (c2[j,m_i] - c2[j - 1,m_i])
                                / (h2s * x2d_nodes[j,m_i] * x2dmh) 
                                +0.5 * (c2[j,m_i] - c2[j,m_i]) / (hls) # left ride contributions
                                - 0.5
                                * (c2[j,m_i] - c2[j,m_i-1])
                                / (hls),  # D_j
                                0.5
        ...
        print(len(val),len(row),len(col),len(b),(n3 + n2 * nv) * m)
        print(40 + 1052*2 + 38 * 49 + 38 + 38 + 38*999 + 38*1001 + 38*1001)

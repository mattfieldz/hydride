...
while t < tmax:
    ...
    if step % 100 == 0:
        ...
        x1_array_log = np.log(np.array(x1)*Lref)
        log_tspace_100 = np.log(np.array(tspace)[::100]*Lref**2/(Dref))
        tspace_100 = np.array(tspace)[::100]*Lref**2/(Dref)
        pcm_conc = axis[0,0].plot(log_tspace_100,x1_array_log)
        plot_A = axis[0,0].plot(log_tspace_100,np.log(np.sqrt(2*A*(1-c_sol)))+0.5*log_tspace_100)

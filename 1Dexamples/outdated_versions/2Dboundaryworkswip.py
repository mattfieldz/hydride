...
while t < tmax:
        ...
        for m_i in range(0,m):
            ...
            if (m_i == 0) or (m_i == m-1):
                if m_i == m-1:
                    for j in range(0,n2+n3):
                        row += 3 * [e1 * j + m_s]
                        col += [e1 * (j-2*m) + m_s, e1 * (j-m) + m_s, e1 * (j)+m_s]
                        val += [-1, 4, -3]
                        b += [c[e1*(j),m_i-2] + 3*c[e1*j,m_i] - 4*c[e1*(j),m_i-1]]
                    
                        row += 3*[e1 * j + 1+m_s]
                        col += [e1 * (j-2*m) + 1 + m_s, e1* (j-m) + 1 + m_s, e1 * (j) + 1 + m_s]
                        val += [-1,4,-3]
            ...
            # internal nodes
                ...
                for j in range(1, n3 - 1):
                    ...
                    row += 3 * [e1*j+m_s] # this is [j,j,j]
                    col += [e1*(j-1)+m_s, e1*j+m_s, e1*(j+1)+m_s]
                    val += [D3 / h3s, -2 * D3 / h3s - 1 / dt, D3 / h3s]
                    b += [-c3_nodes[e1*j,m_i] / dt - D3/h3s * c3[e1*(j-1),m_i] + (1/dt+2*D3/h3s) * c3[e1*j,m_i]
                        - D3/h3s * c3[e1*(j+1),m_i]]
                    # b+=[0]
                    # metal nodes
                    row += [e1 * j + 1+m_s]
                    col += [e1 * j + 1+m_s]
            ...
            # oxide region
                ...
                for j in range(1, n2 - 1):  # logical node in metal
                    ...
                    row += 6 * [e1 * k + m_s]
                    col += [e1 * (k-1) + m_s, e1 * k + m_s, e1* k +1 + m_s, e1* (k+1) + m_s]
                    val += [D_neg_half / h2s,
                            -(D_pos_half + D_neg_half) / h2s - 1 / dt - 
                    ...
                    b += [-c2_nodes[e1 * j,m_i] / dt + 
                        ...
                        -(D_neg_half / h2s) * c2[e1*(j-1),m_i] + (1/dt + (D_pos_half + D_neg_half) / h2s)* c2[e1*j,m_i] 
    ...
    # print(max(row))
        ...
        # arra = a.toarray()
        # if arra[0:16].all() == arra[16:32].all():
        #     print('good')
        # print(arra[:,0])
        # print('determinant',np.linalg.det(arra[16:32,16:32]))
    ...
    if step % 100 == 0:
        ...
        pcm_conc = axis[0,0].plot(length_vector,x[0::2,0])
        # pcm_conc = axis[0,0].plot(np.sqrt(np.array(tspace)[::100]),x1)

        pcm_alpha = axis[0,1].plot(length_vector,1-x[1::2,0])

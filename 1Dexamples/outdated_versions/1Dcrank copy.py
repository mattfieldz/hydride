...
while t < tmax:
    ...
    while tau > 1e-8:  
        ...
        for j in range(1, n3 - 1):
            ...
            b += [2 * (-c3old[e1*j] + c3[e1*j]) / dt + 
                  D3/h3s * (-c3[e1*(j-1)] + 2*c3[e1*j] -c3[e1*(j+1)]) + 
                  D3/h3s * (-c3old[e1*(j-1)] + 2*c3old[e1*j]
                  -c3old[e1*(j+1)])]
            # b+=[0]
            # metal nodes
            row += [e1 * j + 1]
    ...
    # oxide region
        ...
        row += 2 * [e1 * j + 1]
        col += [e1 * j,e1 * j + 1]
        val += [3*epsilon * k_2 *(c2[0]-c_sol)**2 * (1-c2[1]),2/dt + epsilon * reac_func(c2[0],k_2,c_sol)]
        b += [2 * (c2_nodes[1]-c2[1])/dt + (1-c2[1]) * epsilon * reac_func(c2[0],k_2,c_sol) +
              (1-c2old[1]) * epsilon * reac_func(c2old[0],k_2,c_sol)]
        ...
        for j in range(1, n2 - 1):  # logical node in metal
            ...
            D_vectorold = D2 * (1-c2old[1::2]) + D1 * c2old[1::2]
            D_pos_halfold = 0.5 * (D_vectorold[j+1]+D_vectorold[j])
            D_neg_halfold = 0.5 * (D_vectorold[j]+D_vectorold[j-1])  


            row += 4 * [e1 * k]
            col += [e1 * (k-1), e1*k,e1*k+1, e1*(k+1)]
            val += [D_neg_half / (h2s * x2d_nodes[j] * x2dmh),
                     -(D_pos_half/x2dph + D_neg_half/x2dmh) / (h2s * x2d_nodes[j]) - 2 / dt - 
                    3 * 3 * (c2[e1*j]-c_sol)**2*k_2*(1-c2[e1*j+1]) * np.heaviside(c2[e1*j]-c_sol,0.5),
                    -3*reac_func(c2[j],k_2,c_sol),
                     D_pos_half / (h2s * x2d_nodes[j] * x2dph)]
            b += [2 * (-c2_nodes[e1 * j] + c2[e1*j])/ dt + 
                  
                  (3*reac_func(c2[e1*j],k_2,c_sol)*(1-c2[e1*j+1])
                  -(D_neg_half / (h2s * x2dmh * x2d_nodes[j])) * c2[e1*(j-1)] + 
                  ((D_pos_half/x2dph + D_neg_half/x2dmh) / (h2s*x2d_nodes[j]))* c2[e1*j] 
                  - (D_pos_half / (h2s* x2dph * x2d_nodes[j])) * c2[e1*(j+1)]) + 
                  
                  (3*reac_func(c2old[e1*j],k_2,c_sol)*(1-c2old[e1*j+1])
                  -(D_neg_halfold / (h2s * x2dmh * x2d_nodes[j])) * c2old[e1*(j-1)] + 
                  ((D_pos_halfold/x2dph + D_neg_halfold/x2dmh) / (h2s*x2d_nodes[j]))* c2old[e1*j] 
                  - (D_pos_halfold / (h2s* x2dph * x2d_nodes[j])) * c2old[e1*(j+1)])]
            # b += [0]
            # b += [-c2_nodes[e1*j]/dt]
            ...
            row += 2*[e1 * k + 1]
            col += [e1 * k ,e1 * k + 1]
            val += [3*epsilon * k_2 *(c2[e1*j]-c_sol)**2 * (1-c2[e1*j+1]),2/dt + epsilon * reac_func(c2[e1*j],k_2,c_sol)]
            b += [2 * (c2old[e1*j+1] - c2[e1*j+1])/dt + (1 - c2[e1*j+1]) * epsilon * reac_func(c2[e1*j],k_2,c_sol) + 
                  (1 - c2old[e1*j+1]) * epsilon * reac_func(c2old[e1*j],k_2,c_sol)]

...
while t < tmax:
    ...
    # oxide region
        ...
        row += [e1 * j + 1]
        col += [e1 * j + 1]
        val += [1/dt + epsilon * reac_func(c2_update_nodes[0],k_2,c_sol)]
        ...
        for j in range(1, n2 - 1):  # logical node in metal
            ...
            val += [D_neg_half / h2s, -(D_pos_half + D_neg_half) / h2s - 1 / dt - 
                    9*reac_func(c2_update_nodes[e1*j],k_2,c_sol)*(1-c2_update_nodes[e1*j+1]),
                     D_pos_half / h2s]
            b += [-c2_nodes[e1 * j] / dt + 
            ...
            row += [e1 * k + 1]
            col += [e1 * k + 1]
            val += [1/dt + epsilon * reac_func(c2_update_nodes[e1*j],k_2,c_sol)]
    ...
    if step % 10 == 0:
        ...
        pcm_beta = axis[1,0].plot(np.linspace(0,t*epsilon*Lref**2/Dref,step),interfaceconc)
        pcm_gamma = axis[1,1].plot(np.linspace(0,t*epsilon*Lref**2/Dref,step),interfacealpha)
        # print(x[0::2])
    # axis.set_title(counter)
        fig.suptitle(t/(Lref**2*epsilon/(Dref)))
        plt.pause(0.01)
    step += 1

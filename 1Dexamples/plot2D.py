...
for t in t_values:
    ...
    pcm_conc = axis[0,0].plot(1000 * length_vector,Castar*c2[0::2,int(m/2)+1],color='blue')
    pcm_conc_1D = axis[0,0].plot(1000 * length_vector,Castar*c2_1D[0::2],color='red')
    
    
    pcm_alpha = axis[0,1].plot(1000 * length_vector,N2star * (1-c2[1::2,int(m/2)+1]),color='blue')
    pcm_alpha_1D = axis[0,1].plot(1000 * length_vector,N2star * (1-c2_1D[1::2]),color='red')



    pcm_beta = axis[1,0].imshow(c2[0::2]*Castar,aspect='auto',extent=[-Lmstar/2 * 1e7,Lmstar/2 * 1e7,-L2star * 1e7,0])
    pcm_gamma = axis[1,1].imshow(c2[1::2]*N2star,aspect='auto',extent=[-Lmstar/2 * 1e7,Lmstar/2 * 1e7,-L2star * 1e7,0])

...
def reac_func(conc, conc_crit,uran,k):
    reac = k * np.heaviside(conc-conc_crit,0.5) * (conc-conc_crit)**3 * uran
    return reac
...
# Crank Nicholson terms
    for i in range(1,n-1):
            for j in range(1,n-1):
                c_matrix[i,j] = -3 * reac_func(concentration,conc_crit,metal,k)[i,j] + (1-2*r[i,j])*concentration[i,j] + 1/2 * r[i,j] * (concentration[i,j-1] + concentration[i,j+1] + concentration[i+1,j] + concentration[i-1,j])

...
def matrix_generator(n,r):
    ...
    for i in range(M):
        ...
        # sets the rest of matrix with finite difference scheme
            ...
            diag[i] = 1 + 2*r[i]
            upper_diag[i+1] = -r[i]/2 - (r[i+1]-r[i-1])/8
            upper_diag_n[i+n] = -r[i]/2 - (r[i+n]-r[i-1])/8
            lower_diag[i-1] = -r[i]/2 - (r[i+1]-r[i-1])/8
            lower_diag_n[i-n] = -r[i]/2 -  (r[i+n]-r[i-n])/8
...
# Boundaries
    ...
    c_matrix[0,:] = 1*np.exp(-np.linspace(-5,5,n)**2)
    # c_matrix[0,:] = 1
    c_matrix[1:n,0] = 0
    c_matrix[1:n,n-1] = 0
    c_matrix[n-1,:] = 0
# Crank Nicholson terms
    for i in range(1,n-1):
            for j in range(1,n-1):
                c_matrix[i,j] = -3 * reac_func(concentration,conc_crit,metal,k)[i,j] + (1-2*r[i,j])*concentration[i,j] + 1/2 * r[i,j] * (concentration[i,j-1] + concentration[i,j+1] + concentration[i+1,j] + concentration[i-1,j]) + 1/8 *( (r[i+1,j]-r[i-1,j])*(concentration[i+1,j]-concentration[i-1,j]) + (r[i,j+1]-r[i,j-1])*(concentration[i,j+1]-concentration[i,j-1])) 
...
# oxide[0:interface_grid_value,int(n/2)-5:int(n/2)+5] = 0
surface[0:interface_grid_value,int(n/2)-5:int(n/2)+5] = 1
...
axis[1,1].set_xlabel('log t')
axis[1,1].set_ylabel('log %UH3')
...
while counter < total_time:
    ...
    # diffusivity_matrix = D_oxide * oxide + D_hydride * hydride + D_metal * metal + D_surface * surface
    ...
    hydride_count.append(L**2*np.sum(hydride)/(n**2))
    counter += dt
    count += 1
    print('total U',np.sum(hydride+metal+oxide)/n**2,'count=',count)
    pcm_conc.set_array(concentration)
    pcm_uranium.set_array(metal)
    pcm_uranium_hydride.set_array(hydride)
    axis[1,1].cla()
    axis[1,1].set_xlabel('log t')
    axis[1,1].set_ylabel('%UH3')
    axis[1,1].plot(np.log(np.linspace(0,counter,count)),hydride_count)
